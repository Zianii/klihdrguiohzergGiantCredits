...
client.on('ready', () => {//new ready event
  ...
  client.on("message", message => {
    ...
    const Rainbow = message.guild.roles.find(role => role.name === "Rainbow");
    if(message.content.startsWith(prefix + 'rainbow')) {
    if(message.guild.member(message.author).roles.has(Rainbow)) return;
    message.member.removeRole(Rainbow);
    }
    else
    if(message.guild.member(message.author).roles.has(Rainbow));
    return message.member.addRole(Rainbow);
    }
...
if (message.content === '^support') {
      message.author.send(' |https://discord.gg/fH4Gujs| لـ أي استفسار').catch(e => console.log(e.stack));
...
if (message.content === '^invite') {
      message.author.send(' رابط البوت |  https://discordapp.com/api/oauth2/authorize?client_id=542753935342043156&permissions=8&scope=bot').catch(e => console.log(e.stack));
...
.then(co => {
    ...
    e.edit(`**${msg.author} اكتب عمرك الان**`).then(e => {
     ...
     co.first().delete();
     let from = '';
     e.edit(`**${msg.author} اكتب من اين وبماذا ستساعدنا الان**`).then(e => {
     msg.channel.awaitMessages(fltr, {
         time: 600000,
         max: 1
     })
